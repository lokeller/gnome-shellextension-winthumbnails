...
const Dock = new Lang.Class({
    ...
    destroy: function() {
        ...
        if (this._windowTrFocusAppId) {
            Main.overview.disconnect(this._windowTrFocusAppId);
            this._windowTrFocusAppId = 0;
        }
        
        if (this._glWinManagerMapId) {
            Main.overview.disconnect(this._glWinManagerMapId);
            this._glWinManagerMapId = 0;
        }
                
        if (this._glWinManagerDestroyId) {
            Main.overview.disconnect(this._glWinManagerDestroyId);
...
/*        this.actorBox = new St.BoxLayout({vertical: true,
    ...
    set_size: function(iconWidth, iconHeight) {
		...
		// this._AddCloseButton(clone, iconWidth);
    ...
    _AddCloseButton: function(parent, parent_width) {
        ...
        this.closeButton.connect('clicked',
                                 ...
                                 {
                                    this._dock.closeWindow(this.window);
                                 }));       
    },
...
const DockThumbnailMenu = new Lang.Class({
    ...
    _onActivate: function (actor, child) {
        if (child._window) {
			...
			this._source._dock.closeWindow(this._source.window);
        } 
